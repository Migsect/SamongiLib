...
public class PlayerUtil
{
  ...
  /**
   * Gets the block the player is looking at til the max-distance. If there is
   * no block and everything is just air, this will return null.
   * 
   * @param player
   * @param max_distance
   * @return
  ...
  public static Block getTargetedBlock(Player player, double max_distance)
  {
    SamVector direction = new SamVector(player.getLocation().getDirection())
        .normalize();
    SamVector player_location = new SamVector(player.getEyeLocation()
        .toVector());
    ...
    {
      Block found_block = direction.multiply(scalar).add(player_location)
  ...
  /**
   * Gets the air block the player would be looking at. This air block occurs
   * before there is a solid block.
   * 
