...
public class BlockUtil
{
   ...
   * 
   * @param block
   *          The block to find all connecting blocks
   ...
   * 
   * @param block
   *          The block that is being checked around.
   * @param found_blocks
   *          Blocks that should not be included in the checks.
   */
  ...
  public static boolean hasBlocksBetween(Location loc1, Location loc2)
  {
    ...
    BlockIterator iterator = new BlockIterator(loc1.getWorld(), starting_vector, direction_vector, 0,
        (int) Math.ceil(loc1.toVector().distance(loc2.toVector())));
