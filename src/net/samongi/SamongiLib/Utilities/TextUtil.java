...
public class TextUtil
{
  ...
  public static String toKey(String str){return str.toUpperCase().replace(' ', '_').replace('-', '_');}
	...
	static public List<String> formatString(List<String> strings)
	{
		...
		{
			new_strings.add(TextUtil.formatString(s));
		}
		return new_strings;
	}
     ...
     * @param wrappedTab The amount of white space that is added on tabbed lines.
    ...
    public static List<String> wrapText(String text, int characters, int wrappedTab)
    {
        ...
        if(queryCharacter != ' ') {
            wrappedText.add(text.substring(0, characters));
            String remainder = text.substring(characters);
            ...
            if(wrappedSubStrings.size() > 1) {
                wrappedText.addAll(wrappedSubStrings.subList(1, wrappedSubStrings.size()));
            }
            return wrappedText;
        }
